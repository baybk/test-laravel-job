...
class OrderCriteria implements CriteriaInterface
{
    ...
    public function apply($model, RepositoryInterface $repository)
    {
        ...
        foreach ($this->orders as $o) {
            ...
            if (strpos(strtolower($field), 'as unsigned') !== false || strpos(strtoupper($field), 'FIELD(') !== false) {
                $model = $model->orderByRaw($field);
            } else {
                $model = $desc ? $model->orderByRaw('ISNULL(' . $field . ')' . ', ' . $field . ' DESC') : $model->orderBy($field);
            }
        }
